...
def case_create(request, data:CaseIn):
    ...
    newCase = TestCase.objects.create(**data.dict())
...
def case_debug(request, data:CaseDebugIn):
    ...
    if method == "post":
        if params_type == 'form':
            resp = requests.post(url, headers=header, data=json.dumps(params_body)).text
...
def get_case_list(request, module_id: int, **kwargs):
    """获取模块下的用例列表"""
    return TestCase.objects.filter(module_id=module_id, is_delete=False)
